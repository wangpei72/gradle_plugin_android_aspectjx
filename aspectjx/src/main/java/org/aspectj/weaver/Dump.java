...
public class Dump {
	...
	public static void saveCommandLine(String[] args) {
		...
		System.arraycopy(args, 0, commandLine, 0, args.length);
		savedCommandLine.set(commandLine);
	}
	...
	private void dumpFullClasspath() {
		...
		List<String> fullClasspath = savedFullClasspath.get();
		if (fullClasspath != null && fullClasspath.size() > 0) {
			for (String fileName : fullClasspath) {
